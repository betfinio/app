...
const GameBlock: FC<GameBlockProps> = ({className = '', label, online, disabled = true, loading = false}) => {
	return <div
		className={cx(className, disabled && 'grayscale', 'mx-auto col-span-1 cursor-pointer relative w-[137.5px] h-[200px] md:w-[182px] md:h-[265px] bg-contain bg-no-repeat flex flex-col items-center  justify-center gap-2 pt-20 ')}>
		<span className={'md:text-lg font-semibold'}>{label}</span>
		<div className={'flex flex-row items-center gap-2 text-sm md:text-normal'}>
			{disabled ? <Badge className={'px-4 py-1 text-base lowercase'} variant={'outline'}>Soon</Badge> : <>
