...
const ToastViewport = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Viewport>, ToastViewportProps>(
	({ className,fixedSize=true, ...props }, ref) => (
		<ToastPrimitives.Viewport
			ref={ref}
			className={cn(
				'fixed top-0 z-[100] flex affiliate max-h-screen gap-2  flex-col-reverse p-4 sm:bottom-0 sm:right-0 sm:top-auto sm:flex-col ',
				// {"w-full md:max-w-[420px]":fixedSize},
				className,
			)}
			{...props}
		/>
...
const toastVariants = cva(
	'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-4 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full',
	{
		variants: {
			variant: {
				default: 'border bg-primaryLighter border border-green-400 text-white',
				destructive: 'bg-primaryLighter border border-red-roulette text-white group',
				loading: 'bg-primaryLighter border border-yellow-400 text-white',
				soon: 'bg-primaryLighter border border-purple-box text-white',
			},
		},
		defaultVariants: {
			variant: 'default',
